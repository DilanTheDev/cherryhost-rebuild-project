...
import { Server, Clock, Users, Shield, Terminal, Cpu, Database, Gauge, FileIcon, Plugin, Package, Play, Grid, Settings, Shield as FirewallIcon, Globe, Trash } from "lucide-react";

const FeatureCard = ({ icon, title, onClick, isActive }: { icon: React.ReactNode; title: string; onClick: () => void; isActive: boolean }) => {
  return (
    <div 
      className={`glass-card p-4 flex flex-col items-center justify-center gap-3 aspect-square cursor-pointer transition-all ${isActive ? 'ring-2 ring-cherry-500 bg-cherry-900/20' : 'hover:bg-cherry-900/10'}`}
      onClick={onClick}
    >
...
const GamePanel = () => {
  ...
  const panels = [
    ...
    { 
      ...
      icon: <FileIcon className="w-6 h-6 text-cherry-500" />,
    ...
    { 
      ...
      icon: <Package className="w-6 h-6 text-cherry-500" />,
    ...
    { 
      ...
      icon: <Plugin className="w-6 h-6 text-cherry-500" />,
        ...
        <div className="grid md:grid-cols-3 gap-6 mb-12">
          ...
          <div className="md:col-span-2 space-y-4">
            <div className="relative">
              <img 
                src={activeImage} 
                alt={`CherryHost Game Panel - ${panels.find(panel => panel.id === activePanel)?.title}`} 
